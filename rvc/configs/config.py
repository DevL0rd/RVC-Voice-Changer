...
class Config:
    def __init__(self):
        requested_device = os.environ.get("RVC_DEVICE", "auto")
        device_index = int(os.environ.get("RVC_CUDA_DEVICE", "0"))
        if requested_device == "auto":
            requested_device = f"cuda:{device_index}" if torch.cuda.is_available() else "cpu"
        if requested_device.startswith("cuda"):
            ...
            raise RuntimeError(f"Unsupported inference device: {requested_device}")
        self.gpu_name = (
