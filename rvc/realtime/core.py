...
from rvc.realtime.profiling import StageProfiler
...
class Realtime:
    def __init__(
        ...
        **kwargs,
        ...
        self.resample_in = tat.Resample(
            orig_freq=self.audio_sample_rate, new_freq=self.sample_rate, dtype=torch.float32
        ).to(self.device)
    ...
    def realloc(
        ...
        block_frame_16k = int(block_frame / self.audio_sample_rate * self.sample_rate)
        crossfade_frame_16k = int(
            crossfade_frame / self.audio_sample_rate * self.sample_rate
        )
        sola_search_frame_16k = int(
            sola_search_frame / self.audio_sample_rate * self.sample_rate
        )
        extra_frame_16k = int(extra_frame / self.audio_sample_rate * self.sample_rate)
    ...
    def inference(
        ...
        proposed_pitch_threshold: float = 255.0,
        ...
        if self.vad is not None:
            ...
            if profiler:
                profiler.add_cpu("voice_activity_detection", (time.perf_counter() - vad_started) * 1000.0)
            if not is_speech:
...
class VoiceChanger:
    def __init__(
        ...
        **kwargs,
        ...
        self.audio_sample_rate = audio_sample_rate
        self.crossfade_frame = int(cross_fade_overlap_size * self.audio_sample_rate)
        self.extra_frame = int(extra_convert_size * self.audio_sample_rate)
        self.sola_search_frame = self.audio_sample_rate // 100
    ...
    def process_audio(
        ...
        if is_silence:
            ...
            profiler.mark("silence_output")
            self.last_profile = profiler.finish()
