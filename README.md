...
Packages (for example from the AUR) pass `--aur` or set `RVC_AUR=true` so the package manager handles updates instead, and no hook is registered. `./uninstall.sh` removes the hook.
